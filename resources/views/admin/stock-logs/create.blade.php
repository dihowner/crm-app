    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="page-title mb-1">Add Stock Log</h4>
                    <p class="text-muted mb-0">Record a new stock movement or inventory change</p>
                </div>
                <div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('action') is-invalid @enderror"
                                    ...
                                    @foreach($actions as $action)
                                        <option value="{{ $action }}" {{ old('action', 'Add Stock') == $action ? 'selected' : '' }}>
                                            {{ $action }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('comment') is-invalid @enderror"
                                          ...
                                          placeholder="Add any additional notes or comments about this stock movement...">{{ old('comment') }}</textarea>
                                @error('comment')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                <div class="d-flex gap-2">
                                    <button type="submit" class="btn btn-primary">
                                        <i class="ti ti-device-floppy me-2"></i>Create Stock Log
                                    </button>
                                    <a href="{{ route('admin.stock-logs.index') }}" class="btn btn-secondary">
...
<script>
// Auto-populate created_by with current user if available
document.addEventListener('DOMContentLoaded', function() {
    // You can add logic here to auto-select the current user
    // if they are logged in and available in the users list
});
</script>
@endsection
